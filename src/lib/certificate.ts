...
export const generateCertificateHTML = (data: CertificateDisplayData): string => {
  return `
    <div style="
      width: 100%;
      height: 100%;
    ...
    ">
      ...
      <div style="text-align: center; margin-bottom: 40px;">
        <p style="color: #4B5563; margin-bottom: 10px;">This is to certify that</p>
        <h2 style="font-size: 28px; font-weight: bold; color: #1F2937; margin-bottom: 10px; font-family: serif;">${data.studentName}</h2>
        <p style="color: #4B5563; margin-bottom: 20px;">has successfully completed the course</p>
        <h3 style="font-size: 24px; font-weight: bold; color: #2563EB; margin-bottom: 10px;">"${data.courseName}"</h3>
        <p style="color: #4B5563;">
          on ${data.issueDate}
        </p>
      </div>
      ...
      <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 80px;">
        ...
        <div style="text-align: center;">
          ...
          />
          <p style="font-size: 12px; color: #6B7280;">Scan to verify</p>
          <p style="font-size: 12px; color: #6B7280;">${data.certificateId}</p>
        </div>
...
export const certificateHtmlToImage = async (htmlElement: HTMLElement): Promise<string> => {
  try {
    ...
    throw new Error('Failed to convert certificate to image');
  }
};
